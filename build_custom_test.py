...
# Get PyPI test account token
PYPI_TEST_ACC = os.getenv('PYPI_TEST_ACC')
if not PYPI_TEST_ACC:
    raise EnvironmentError("Please set PYPI_TEST_ACC environment variable with your TestPyPI token")
...
def main():
    ...
    try:
        ...
        # Upload
        console.print("\n[yellow][Uploading][/yellow] to TestPyPI...")
        run_command("python3 -m twine upload --repository testpypi --skip-existing dist/* --verbose")

        console.print(f"\n[green][Success][/green] Version {new_version} built and uploaded!")
